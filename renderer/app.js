...
class App extends Component {
	...
	initIpc=()=>{
		...
		ipc.on('get-config', function(event, config){
			...
			self.setState({count:config['tran_file_no']})
		})
		var self = this;
	...
	computeString =()=>{
		...
		let s = data['tenant_id']+'|'+data['pos_no']+'|'+data['sale_date']+'|'+data['tran_file_no']+'|'+data['open_receipt']+'|'+data['close_receipt']+'|'+data['total_receipt']+'|'+data['gross_sales']+'|'+'|'+'|'+'|'+'|'+'|'+'|'+data['net_sales']
		this.setState({outputString:s})
	}
	...
	saveFileClick = (e) => {
		...
		ipc.send('open-save-dialog',{outputString:this.state.outputString, namePattern:namePattern, fileNo:fileNo})
	}
			...
			<Container fluid>
				<Container as='div' id='header-container' textAlign='center'>
					<Header as='h2' icon>
						...
						POS Table Reader
						<Header.Subheader>
						TENANT ID: { config['tenant_id']? config['tenant_id']:null } Pos Number: { config['pos_no']? config['pos_no']:null } Current File Count: { config['tran_file_no']? config['tran_file_no']:null}
						</Header.Subheader>
					</Header>
  				</Container>
				<Divider/>
